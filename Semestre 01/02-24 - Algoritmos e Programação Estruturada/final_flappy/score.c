...
void adicionar_jogador() {
    ...
    printf("%sDigite o nome do jogador:%s ", AMARELO, LIMPA);
    fflush(stdin);
    ...
    printf("Jogador %s%s%s adicionado com sucesso!\tID: %s%d%s\n", AMARELO,
           novo_jogador.nome, LIMPA, AMARELO, novo_jogador.id, LIMPA);
    getch();
    fwrite(&novo_jogador, sizeof(jogadores_t), 1, file);
    fclose(file);
    numerozao();
    int pontuacao = flappy(0);
...
void jogar_novamente() {
    ...
    while (fread(&jogador, sizeof(jogadores_t), 1, file)) {
        if (jogador.id == id) {
            ...
            int pontuacao;
            numerozao();
            if (jogador.partidas >= 2) {
                int dific = rand() % 2 ? 1 : 0;
                pontuacao = flappy(dific);
            } else {
                pontuacao = flappy(0);
            }
